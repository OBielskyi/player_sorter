...
class Player:
    ...
    def __init__(self, first_name: str = "", last_name: str = "", nickname: str = "", 
                 rating: int = 0, wins: int = 0, losses: int = 0, draws: int = 0, 
                 byes: int = 0, half_byes: int = 0):
    ...
    def points(self):
        ...
        return self.wins * 1.0 + self.draws * 0.5 + self.byes * 1.0 + self.half_byes * 0.5
    ...
    def from_dict(cls, data):
        player = cls(data['name'], data['rating'], data.get('wins', 0), 
                    data.get('losses', 0), data.get('draws', 0), data.get('byes', 0),
...
class PlayerSorterApp:
    ...
    def apply_theme(self, theme_name):
        ...
        style.configure('TButton', font=('Arial', 12), padding=10,
                       ...
                       bordercolor=theme['border'])
        style.map('TButton',
                 background=[('active', theme['select_bg']), ('pressed', theme['highlight'])],
        ...
        style.configure('Large.TButton', font=('Arial', 14, 'bold'), padding=15,
                       background=theme['accent_button_bg'], foreground=theme['accent_button_fg'])
        style.map('Large.TButton',
                 background=[('active', theme['highlight']), ('pressed', theme['select_bg'])],
        ...
        style.configure('Large.TRadiobutton', font=('Arial', 11), padding=5,
        ...
        style.configure('Treeview', background=theme['entry_bg'], foreground=theme['entry_fg'],
                       fieldbackground=theme['entry_bg'], bordercolor=theme['border'])
    ...
    def show_theme_selection(self):
        ...
        for i, theme_name in enumerate(THEMES.keys()):
            btn = ttk.Button(theme_frame, text=theme_name, width=25,
                           command=lambda t=theme_name: self.select_theme(t))
            btn.pack(pady=8)
            
            # Highlight current theme
            if theme_name == self.current_theme:
                ttk.Label(theme_frame, text="✓ Current", 
                         font=('Arial', 9, 'italic')).pack()
        
        # Continue button
        ttk.Button(frame, text="Continue to App", width=25, style='Large.TButton',
                  command=self.show_initial_selection).pack(pady=30)
    ...
    def show_initial_selection(self):
        ...
        ttk.Button(game_frame, text="Chess (ELO)", width=30, style='Large.TButton',
                  command=lambda: self.select_game_type('chess')).pack(side=tk.LEFT, padx=15)
        ttk.Button(game_frame, text="E-Sports (Trophies)", width=30, style='Large.TButton',
                  command=lambda: self.select_game_type('esports')).pack(side=tk.LEFT, padx=15)
        
        # Theme switcher button at bottom
        ttk.Button(frame, text="🎨 Change Theme", width=20,
                  command=self.show_theme_selection).pack(pady=20)
    ...
    def show_mode_selection(self):
        ...
        title = ttk.Label(frame, text=f"{game_name} - Select Mode", font=('Arial', 24, 'bold'))
        title.pack(pady=30)
        ...
        if self.game_type == 'chess':
            modes = [
                ("Tournament", "tournament", "Swiss, Round-Robin, Knockout, Scheveningen"),
        ...
        for mode_name, mode_id, description in modes:
            ...
            btn = ttk.Button(btn_frame, text=mode_name, width=25,
                           command=lambda m=mode_id: self.select_mode(m))
            btn.pack(side=tk.LEFT, padx=10)
            
            ttk.Label(btn_frame, text=f"- {description}", font=('Arial', 11)).pack(side=tk.LEFT, padx=10)
        
        # Back button
        ttk.Button(frame, text="← Back", command=self.show_initial_selection).pack(pady=30)
    ...
    def show_tournament_system_selection(self):
        ...
        # Title with larger font
        title = ttk.Label(frame, text="Chess - Select Tournament System", font=('Arial', 24, 'bold'))
        title.pack(pady=30)
        
        # Tournament systems with larger fonts
        ttk.Label(frame, text="Select Tournament System:", font=('Arial', 14)).pack(pady=20)
        
        systems = [
            ("Swiss System", "swiss", "Players paired by score, avoid repeat opponents"),
        ...
        for sys_name, sys_id, description in systems:
            ...
            btn = ttk.Button(btn_frame, text=sys_name, width=30,
    ...
    def show_tiebreak_selection(self):
        ...
        system_name = "Swiss System" if self.tournament_system == 'swiss' else "Round-Robin"
        title = ttk.Label(frame, text=f"{system_name} - Configuration", font=('Arial', 24, 'bold'))
        title.pack(pady=30)
        
        # Tiebreak selection
        ttk.Label(frame, text="Select Tiebreak Method:", font=('Arial', 14)).pack(pady=20)
        ...
        for tb_name, tb_id, description in tiebreaks:
            ...
            btn = ttk.Button(btn_frame, text=tb_name, width=25,
                           command=lambda t=tb_id: self.set_tiebreak_and_continue(t))
            btn.pack(side=tk.LEFT, padx=10)
            
            ttk.Label(btn_frame, text=f"- {description}", font=('Arial', 11)).pack(side=tk.LEFT, padx=10)
        
        ttk.Button(frame, text="← Back", command=self.show_tournament_system_selection).pack(pady=30)
    ...
    def show_half_bye_option(self):
        ...
        system_name = "Swiss System" if self.tournament_system == 'swiss' else "Round-Robin"
        title = ttk.Label(frame, text=f"{system_name} - Tournament Settings", font=('Arial', 22, 'bold'))
        title.pack(pady=20)
        ...
        rating_frame = ttk.LabelFrame(scrollable_frame, text="Rating Changes", padding="20", 
        ...
        if self.game_type == 'chess':
            ...
            ttk.Radiobutton(rating_frame, text="Automatic - Online/OTB (balanced changes, K=32)", 
                           variable=self.rating_mode_var, value="automatic_otb",
                           style='Large.TRadiobutton').pack(anchor=tk.W, pady=4)
            ttk.Radiobutton(rating_frame, text="Automatic - Daily/Correspondence (harsher changes, K=48)", 
                           variable=self.rating_mode_var, value="automatic_correspondence",
                           style='Large.TRadiobutton').pack(anchor=tk.W, pady=4)
            ttk.Radiobutton(rating_frame, text="Manual - Manually update ELO after each round", 
                           variable=self.rating_mode_var, value="manual",
                           style='Large.TRadiobutton').pack(anchor=tk.W, pady=4)
            ttk.Radiobutton(rating_frame, text="Unranked - No ELO changes", 
                           variable=self.rating_mode_var, value="unranked",
                           style='Large.TRadiobutton').pack(anchor=tk.W, pady=4)
        else:  # esports
            ttk.Label(rating_frame, 
            ...
            ttk.Radiobutton(rating_frame, text="Ranked - Manually update trophies after each round", 
                           variable=self.rating_mode_var, value="ranked",
                           style='Large.TRadiobutton').pack(anchor=tk.W, pady=4)
            ttk.Radiobutton(rating_frame, text="Unranked - No trophy changes", 
                           variable=self.rating_mode_var, value="unranked",
                           style='Large.TRadiobutton').pack(anchor=tk.W, pady=4)
        ...
        ttk.Label(hb_frame, 
                 ...
                 font=('Arial', 11), wraplength=700).pack(pady=8)
        
        self.half_bye_var = tk.BooleanVar(value=False)
        ttk.Radiobutton(hb_frame, text="Yes - Enable half-byes", 
                       variable=self.half_bye_var, value=True,
                       style='Large.TRadiobutton').pack(anchor=tk.W, pady=4)
        ttk.Radiobutton(hb_frame, text="No - Disable half-byes", 
                       variable=self.half_bye_var, value=False,
                       style='Large.TRadiobutton').pack(anchor=tk.W, pady=4)
        
        # Withdrawal Option - larger
        wd_frame = ttk.LabelFrame(scrollable_frame, text="Player Withdrawals", padding="20")
        ...
        ttk.Radiobutton(wd_frame, text="Yes - Allow withdrawals", 
                       variable=self.withdrawal_var, value=True,
                       style='Large.TRadiobutton').pack(anchor=tk.W, pady=4)
        ttk.Radiobutton(wd_frame, text="No - No withdrawals", 
                       variable=self.withdrawal_var, value=False,
                       style='Large.TRadiobutton').pack(anchor=tk.W, pady=4)
        
        # Max Rounds Option - larger
        rounds_frame = ttk.LabelFrame(scrollable_frame, text="Maximum Rounds", padding="20")
        ...
        ttk.Label(rounds_input_frame, text="Rounds:", font=('Arial', 11)).pack(side=tk.LEFT, padx=8)
        ttk.Entry(rounds_input_frame, textvariable=self.max_rounds_var, width=15, 
                 font=('Arial', 11)).pack(side=tk.LEFT, padx=8)
        ttk.Label(rounds_input_frame, text="(leave empty for unlimited)", 
                 font=('Arial', 10, 'italic')).pack(side=tk.LEFT, padx=8)
        
        # ELO Limits (Chess only) - larger
        if self.game_type == 'chess':
            elo_frame = ttk.LabelFrame(scrollable_frame, text="ELO Requirements", padding="20")
            ...
            ttk.Label(min_elo_frame, text="Minimum ELO:", font=('Arial', 11)).pack(side=tk.LEFT, padx=8)
            self.min_elo_var = tk.StringVar(value="1000")
            ttk.Entry(min_elo_frame, textvariable=self.min_elo_var, width=15,
                     font=('Arial', 11)).pack(side=tk.LEFT, padx=8)
            ttk.Label(min_elo_frame, text="(default: 1000, absolute minimum: 100)", 
                     font=('Arial', 10, 'italic')).pack(side=tk.LEFT, padx=8)
            ...
            ttk.Label(max_elo_frame, text="Maximum ELO:", font=('Arial', 11)).pack(side=tk.LEFT, padx=8)
            self.max_elo_var = tk.StringVar(value="")
            ttk.Entry(max_elo_frame, textvariable=self.max_elo_var, width=15,
                     font=('Arial', 11)).pack(side=tk.LEFT, padx=8)
            ttk.Label(max_elo_frame, text="(leave empty for no upper limit)", 
                     font=('Arial', 10, 'italic')).pack(side=tk.LEFT, padx=8)
        ...
        ttk.Button(btn_frame, text="← Back", width=15,
                  command=self.show_tiebreak_selection).pack(side=tk.LEFT, padx=10)
        ttk.Button(btn_frame, text="Continue to Setup", width=20,
                  command=self.confirm_tournament_settings).pack(side=tk.LEFT, padx=10)
        ...
        if max_rounds_str:
            try:
                ...
                if self.max_rounds < 1:
                    messagebox.showwarning("Invalid Input", "Maximum rounds must be at least 1")
                    return
            except ValueError:
                messagebox.showwarning("Invalid Input", "Maximum rounds must be a number")
                return
        else:
            self.max_rounds = None
        ...
        if self.game_type == 'chess':
            ...
            if min_elo_str:
                try:
                    ...
                    if self.min_elo < 100:
                        messagebox.showwarning("Invalid Input", "Minimum ELO cannot be below 100")
                        return
                except ValueError:
                    messagebox.showwarning("Invalid Input", "Minimum ELO must be a number")
                    return
            else:
            ...
            if max_elo_str:
                try:
                    ...
                    messagebox.showwarning("Invalid Input", "Maximum ELO must be a number")
                    return
            else:
    ...
    def show_scheveningen_setup(self):
        ...
        title = ttk.Label(frame, text="Scheveningen System - Setup", font=('Arial', 16, 'bold'))
        title.pack(pady=20)
        
        ttk.Label(frame, text="In Scheveningen, two teams compete.", font=('Arial', 11)).pack(pady=5)
        ttk.Label(frame, text="Every player from Team A plays every player from Team B.", 
                 font=('Arial', 10)).pack(pady=5)
        
        ttk.Label(frame, text="\nPlayers per team:", font=('Arial', 11, 'bold')).pack(pady=10)
        
        self.schev_team_size_var = tk.IntVar(value=4)
        spinbox = ttk.Spinbox(frame, from_=2, to=10, textvariable=self.schev_team_size_var, width=10)
        spinbox.pack(pady=5)
        
        ttk.Label(frame, text="\nTiebreak method:", font=('Arial', 11, 'bold')).pack(pady=10)
        ...
        for tb_name, tb_id in tiebreaks:
            ttk.Radiobutton(frame, text=tb_name, variable=self.schev_tiebreak_var, 
                          value=tb_id).pack(pady=2)
        
        btn_frame = ttk.Frame(frame)
        btn_frame.pack(pady=20)
        
        ttk.Button(btn_frame, text="← Back", 
                  command=self.show_tournament_system_selection).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Continue", 
                  command=self.confirm_scheveningen_setup).pack(side=tk.LEFT, padx=5)
    ...
    def show_knockout_settings(self):
        ...
        title = ttk.Label(frame, text="Knockout - Tournament Settings", font=('Arial', 16, 'bold'))
        title.pack(pady=20)
        ...
        if self.game_type == 'chess':
            ...
            ttk.Radiobutton(rating_frame, text="Automatic - Online/OTB (balanced changes, K=32)", 
                           variable=self.rating_mode_var, value="automatic_otb").pack(anchor=tk.W, pady=2)
            ttk.Radiobutton(rating_frame, text="Automatic - Daily/Correspondence (harsher changes, K=48)", 
                           variable=self.rating_mode_var, value="automatic_correspondence").pack(anchor=tk.W, pady=2)
            ttk.Radiobutton(rating_frame, text="Manual - Manually update ELO after each round", 
                           variable=self.rating_mode_var, value="manual").pack(anchor=tk.W, pady=2)
            ttk.Radiobutton(rating_frame, text="Unranked - No ELO changes", 
                           variable=self.rating_mode_var, value="unranked").pack(anchor=tk.W, pady=2)
        else:  # esports
            ttk.Label(rating_frame, 
                     text="Should trophy ratings be updated?",
                     font=('Arial', 10, 'bold')).pack(pady=5)
            ...
            ttk.Radiobutton(rating_frame, text="Ranked - Manually update trophies after each round", 
                           variable=self.rating_mode_var, value="ranked").pack(anchor=tk.W, pady=2)
            ttk.Radiobutton(rating_frame, text="Unranked - No trophy changes", 
                           variable=self.rating_mode_var, value="unranked").pack(anchor=tk.W, pady=2)
        ...
        ttk.Label(note_frame, text="Knockout tournaments have a fixed format:\n"
        ...
        if self.game_type == 'chess':
            ...
            self.min_elo_var = tk.StringVar(value="1000")
            ttk.Entry(min_elo_frame, textvariable=self.min_elo_var, width=10).pack(side=tk.LEFT, padx=5)
            ttk.Label(min_elo_frame, text="(default: 1000, min: 100)", 
            ...
            self.max_elo_var = tk.StringVar(value="")
            ttk.Entry(max_elo_frame, textvariable=self.max_elo_var, width=10).pack(side=tk.LEFT, padx=5)
            ttk.Label(max_elo_frame, text="(leave empty for no limit)", 
                     font=('Arial', 9, 'italic')).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Button(btn_frame, text="← Back", 
                  command=self.show_tournament_system_selection).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Continue to Setup", 
                  command=self.confirm_knockout_settings).pack(side=tk.LEFT, padx=5)
    ...
    def show_scheveningen_settings(self):
        ...
        title = ttk.Label(frame, text="Scheveningen - Tournament Settings", font=('Arial', 16, 'bold'))
        title.pack(pady=20)
        ...
        # Rating Mode
        rating_frame = ttk.LabelFrame(scrollable_frame, text="Rating Changes", padding="15")
        ...
        self.half_bye_var = tk.BooleanVar(value=False)
        ttk.Radiobutton(hb_frame, text="Yes - Enable half-byes", 
                       variable=self.half_bye_var, value=True).pack(anchor=tk.W, pady=2)
        ttk.Radiobutton(hb_frame, text="No - Disable half-byes", 
                       variable=self.half_bye_var, value=False).pack(anchor=tk.W, pady=2)
        
        # Withdrawals (Scheveningen supports this)
        wd_frame = ttk.LabelFrame(scrollable_frame, text="Player Withdrawals", padding="15")
        ...
        self.withdrawal_var = tk.BooleanVar(value=False)
        ttk.Radiobutton(wd_frame, text="Yes - Allow withdrawals", 
                       variable=self.withdrawal_var, value=True).pack(anchor=tk.W, pady=2)
        ttk.Radiobutton(wd_frame, text="No - No withdrawals", 
                       variable=self.withdrawal_var, value=False).pack(anchor=tk.W, pady=2)
        
        # Note about fixed rounds
        note_frame = ttk.LabelFrame(scrollable_frame, text="Tournament Length", padding="10")
        note_frame.pack(pady=10, padx=20, fill=tk.X)
        ttk.Label(note_frame, text=f"Scheveningen has fixed rounds:\n"
        ...
        # ELO Limits (Chess only)
        if self.game_type == 'chess':
            elo_frame = ttk.LabelFrame(scrollable_frame, text="ELO Requirements", padding="15")
        ...
        ttk.Button(btn_frame, text="← Back", 
                  command=self.show_scheveningen_setup).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Continue to Setup", 
                  command=self.confirm_scheveningen_settings).pack(side=tk.LEFT, padx=5)
    ...
    def show_player_input(self):
        ...
        title = ttk.Label(main_frame, text=f"{game_name} - {mode_name}", font=('Arial', 20, 'bold'))
        title.pack(pady=15)
        ...
        ttk.Label(input_frame, text="First Name:", font=('Arial', 11)).grid(row=0, column=0, sticky=tk.W, padx=10, pady=8)
        ...
        ttk.Label(input_frame, text="Last Name:", font=('Arial', 11)).grid(row=0, column=2, sticky=tk.W, padx=10, pady=8)
        self.last_name_entry = ttk.Entry(input_frame, width=20, font=('Arial', 11))
        self.last_name_entry.grid(row=0, column=3, padx=10, pady=8)
        
        ttk.Label(input_frame, text="Nickname:", font=('Arial', 11)).grid(row=1, column=0, sticky=tk.W, padx=10, pady=8)
        self.nickname_entry = ttk.Entry(input_frame, width=20, font=('Arial', 11))
        self.nickname_entry.grid(row=1, column=1, padx=10, pady=8)
        
        # Rating input
        ttk.Label(input_frame, text=f"{rating_name}:", font=('Arial', 11)).grid(row=1, column=2, sticky=tk.W, padx=10, pady=8)
        self.rating_entry = ttk.Entry(input_frame, width=15, font=('Arial', 11))
        self.rating_entry.grid(row=1, column=3, padx=10, pady=8)
        
        # Add/Update button
        self.add_update_btn = ttk.Button(input_frame, text="Add Player", command=self.add_or_update_player)
        ...
        req_label = ttk.Label(input_frame, text=req_text, font=('Arial', 9, 'italic'), foreground='blue')
        ...
        columns = ['name', 'rating', 'wins', 'losses', 'draws', 'byes', 'hbyes', 'winrate']
        ...
        ttk.Button(button_frame, text="Edit Selected", command=self.edit_player).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Remove Selected", command=self.remove_player).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Clear All", command=self.clear_players).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Save Players", command=self.save_players).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Load Players", command=lambda: self.load_players() and self.refresh_player_list()).pack(side=tk.LEFT, padx=5)
        
        # Action buttons
        action_frame = ttk.Frame(main_frame)
        action_frame.pack(fill=tk.X, padx=10, pady=5)
        
        ttk.Button(action_frame, text="← Back", command=self.show_mode_selection).pack(side=tk.LEFT, padx=5)
        ttk.Button(action_frame, text="Start Game", command=self.start_game,
    ...
    def add_or_update_player(self):
        ...
        if is_chess and is_tournament:
            ...
            if not nickname:
                messagebox.showwarning("Input Error", 
                    "E-Sports mode requires a Nickname")
                return
        ...
        if self.game_type == 'chess':
            ...
            if self.sort_mode == 'tournament':
                if self.min_elo and rating < self.min_elo:
                    messagebox.showwarning("Invalid Rating", 
    ...
    def remove_player(self):
        ...
        selected = self.tree.selection()
        if not selected:
            messagebox.showwarning("Selection Error", "Please select a player to remove")
            return
    ...
    def load_players(self):
        ...
        try:
            ...
            if data.get('game_type') != self.game_type:
                messagebox.showwarning("Wrong Game Type", 
    ...
    def show_simple_rating_mode_selection(self):
        ...
        mode_name = self.sort_mode.replace('_', ' ').title()
        title = ttk.Label(frame, text=f"{mode_name} - Rating Settings", font=('Arial', 16, 'bold'))
        title.pack(pady=20)
        ...
        if self.game_type == 'chess':
            ...
            ttk.Radiobutton(rating_frame, text="Automatic - Online/OTB (balanced changes, K=32)", 
                           variable=self.rating_mode_var, value="automatic_otb").pack(anchor=tk.W, pady=5, padx=20)
            ttk.Radiobutton(rating_frame, text="Automatic - Daily/Correspondence (harsher changes, K=48)", 
                           variable=self.rating_mode_var, value="automatic_correspondence").pack(anchor=tk.W, pady=5, padx=20)
            ttk.Radiobutton(rating_frame, text="Manual - Manually update ELO after each round", 
                           variable=self.rating_mode_var, value="manual").pack(anchor=tk.W, pady=5, padx=20)
            ttk.Radiobutton(rating_frame, text="Unranked - No ELO changes", 
                           variable=self.rating_mode_var, value="unranked").pack(anchor=tk.W, pady=5, padx=20)
        else:  # esports
            ttk.Label(rating_frame, 
            ...
            ttk.Radiobutton(rating_frame, text="Ranked - Manually update trophies after each round", 
                           variable=self.rating_mode_var, value="ranked").pack(anchor=tk.W, pady=5, padx=20)
            ttk.Radiobutton(rating_frame, text="Unranked - No trophy changes", 
                           variable=self.rating_mode_var, value="unranked").pack(anchor=tk.W, pady=5, padx=20)
        ...
        if self.sort_mode in ['dual', 'teams']:
            ...
            ttk.Entry(rounds_input_frame, textvariable=self.max_rounds_var, width=10).pack(side=tk.LEFT, padx=5)
            ttk.Label(rounds_input_frame, text="(leave empty for unlimited)", 
                     font=('Arial', 9, 'italic')).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Button(btn_frame, text="← Back", 
                  command=self.show_player_input).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Start", 
                  command=self.confirm_simple_rating_mode).pack(side=tk.LEFT, padx=5)
    ...
    def confirm_simple_rating_mode(self):
        ...
        if self.sort_mode == 'dual':
            if len(self.players) < 2:
                messagebox.showwarning("Not Enough Players", "Need at least 2 players for dual mode")
                return
            self.show_dual_game()
        elif self.sort_mode == 'battle_royale':
            if len(self.players) < 4:
                messagebox.showwarning("Not Enough Players", "Need at least 4 players for battle royale")
                return
    ...
    def show_dual_game(self):
        ...
        active_players.sort(
            key=lambda p: (p.rating * 0.6 + p.win_rate * 10 * 0.4) + random.randint(-50, 50), 
            reverse=True
        )
        ...
        # Title
        title = ttk.Label(frame, text=f"Dual Mode - Round {self.current_round}", 
                         font=('Arial', 18, 'bold'))
        title.pack(pady=10)
        ...
        for i, pair in enumerate(pairs):
            p1, p2 = pair[0], pair[1]
            pair_frame = ttk.LabelFrame(scrollable_frame, text=f"Match {i+1}", padding="10")
            pair_frame.pack(fill=tk.X, padx=5, pady=5)
            
            # Player 1
            p1_label = f"{p1.name} ({rating_name}: {p1.rating}, WR: {p1.win_rate:.1f}%)"
            ttk.Label(pair_frame, text=p1_label, font=('Arial', 10)).grid(row=0, column=0, sticky=tk.W, padx=5)
            
            # VS
            ttk.Label(pair_frame, text="vs", font=('Arial', 10, 'italic')).grid(row=0, column=1, padx=10)
            
            # Player 2
            p2_label = f"{p2.name} ({rating_name}: {p2.rating}, WR: {p2.win_rate:.1f}%)"
            ttk.Label(pair_frame, text=p2_label, font=('Arial', 10)).grid(row=0, column=2, sticky=tk.W, padx=5)
            ...
            ttk.Radiobutton(result_frame, text=f"{p1.name} Wins", 
                          variable=result_var, value="p1_win").pack(side=tk.LEFT, padx=5)
            ttk.Radiobutton(result_frame, text="Draw", 
                          variable=result_var, value="draw").pack(side=tk.LEFT, padx=5)
            ttk.Radiobutton(result_frame, text=f"{p2.name} Wins", 
                          variable=result_var, value="p2_win").pack(side=tk.LEFT, padx=5)
        
        if leftover:
            leftover_frame = ttk.LabelFrame(scrollable_frame, text="Bye (1 point)", padding="10")
            ...
            self.dual_results.append(([leftover, None, "bye"], tk.StringVar(value="bye")))
        ...
        ttk.Button(btn_frame, text="← Back to Setup", 
                  command=self.back_to_setup).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Finish Round", 
                  command=self.finish_dual_round).pack(side=tk.LEFT, padx=5)
    ...
    def finish_dual_round(self):
        ...
        for pair, result_var in self.dual_results:
            if result_var.get() == "" and result_var.get() != "bye":
                messagebox.showwarning("Incomplete", "Please set results for all matches")
                return
        ...
        elif self.rating_mode == "manual" or (self.rating_mode == "ranked" and self.game_type == 'esports'):
    ...
    def show_dual_standings(self):
        ...
        # Title
        title = ttk.Label(frame, text=f"Dual Mode - Standings After Round {self.current_round}", 
                         font=('Arial', 18, 'bold'))
        title.pack(pady=10)
        ...
        tree = ttk.Treeview(results_frame, 
                           columns=['rank', 'name', 'rating', 'wins', 'losses', 'draws', 'byes', 'hbyes', 'winrate'], 
        ...
        for i, player in enumerate(sorted_players, 1):
            tree.insert('', tk.END, values=(
                i, player.name, player.rating, player.wins, player.losses, 
                player.draws, player.byes, player.half_byes, f"{player.win_rate:.1f}%"
            ))
        ...
        if self.max_rounds and self.current_round >= self.max_rounds:
            ...
            ttk.Button(btn_frame, text="View Final Standings",
                      command=self.show_dual_final_standings).pack(side=tk.LEFT, padx=5)
        else:
            # Can continue
            ttk.Button(btn_frame, text="Next Round", 
                      command=self.next_dual_round).pack(side=tk.LEFT, padx=5)
            ttk.Button(btn_frame, text="Finish Tournament",
                      command=self.show_dual_final_standings).pack(side=tk.LEFT, padx=5)
    ...
    def show_dual_final_standings(self):
        ...
        # Title
        title = ttk.Label(frame, text=f"Dual Mode - Final Standings", 
                         font=('Arial', 18, 'bold'))
        title.pack(pady=10)
        ...
        if sorted_players:
            ...
            ttk.Label(frame, text=f"🏆 Winner: {winner.name} 🏆",
        ...
        # Button
        ttk.Button(frame, text="← Back to Setup", 
                  command=self.back_to_setup).pack(pady=10)
    ...
    def show_battle_royale_game(self):
        ...
        # Instructions
        ttk.Label(frame, text="Add wins, losses, or draws for each player, then finish the round.", 
                 font=('Arial', 10)).pack(pady=5)
        ttk.Label(frame, text="Bottom 3 players will be eliminated after each round.", 
        ...
        self.br_tree = ttk.Treeview(table_frame, columns=columns[:-1], show='headings', height=12)
        ...
        for player in active_players:
            self.br_tree.insert('', tk.END, values=(
                player.name, player.rating, player.wins, player.losses, 
                player.draws, f"{player.win_rate:.1f}%"
            ))
        
        scrollbar = ttk.Scrollbar(table_frame, orient=tk.VERTICAL, command=self.br_tree.yview)
        ...
        action_frame = ttk.LabelFrame(frame, text="Add Result for Selected Player", padding="10")
        action_frame.pack(fill=tk.X, pady=10)
        
        ttk.Button(action_frame, text="+ Win", 
                  command=lambda: self.add_br_result('win')).pack(side=tk.LEFT, padx=5)
        ttk.Button(action_frame, text="+ Loss", 
                  command=lambda: self.add_br_result('loss')).pack(side=tk.LEFT, padx=5)
        ttk.Button(action_frame, text="+ Draw", 
                  command=lambda: self.add_br_result('draw')).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Button(btn_frame, text="← Back to Setup", 
                  command=self.back_to_setup).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Finish Round", 
                  command=self.finish_br_round).pack(side=tk.LEFT, padx=5)
    ...
    def show_br_elimination(self, eliminated_players: List[Player]):
        ...
        # Title
        title = ttk.Label(frame, text=f"Round {self.current_round} - Eliminations", 
                         font=('Arial', 18, 'bold'))
        title.pack(pady=20)
        ...
        for player in eliminated_players:
            ttk.Label(elim_frame, 
                     text=f"❌ {player.name} (Win Rate: {player.win_rate:.1f}%)", 
                     font=('Arial', 12)).pack(pady=5)
        
        # Remaining count
        remaining = len([p for p in self.players if not p.eliminated])
        ttk.Label(frame, text=f"{remaining} players remaining", 
                 font=('Arial', 11)).pack(pady=10)
        
        # Button
        ttk.Button(frame, text="Continue to Next Round", 
                  command=self.next_br_round).pack(pady=20)
    ...
    def show_br_winner(self):
        ...
        if not active_players:
            active_players = sorted(self.players, key=lambda p: p.win_rate, reverse=True)[:1]
        ...
        rating_name = "ELO" if self.game_type == 'chess' else "Trophies"
        ttk.Label(winner_frame, text=winner.name, font=('Arial', 20, 'bold')).pack(pady=10)
        ttk.Label(winner_frame, text=f"{rating_name}: {winner.rating}", 
                 font=('Arial', 14)).pack(pady=5)
        ttk.Label(winner_frame, text=f"Win Rate: {winner.win_rate:.1f}%", 
                 font=('Arial', 14)).pack(pady=5)
        ...
        ttk.Button(btn_frame, text="← Back to Setup", 
                  command=self.back_to_setup).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="View Final Standings", 
                  command=self.show_br_final_standings).pack(side=tk.LEFT, padx=5)
    ...
    def show_br_final_standings(self):
        ...
        title = ttk.Label(frame, text="Battle Royale - Final Standings", 
                         font=('Arial', 18, 'bold'))
        title.pack(pady=10)
        ...
        tree = ttk.Treeview(results_frame, 
                           columns=['rank', 'name', 'rating', 'wins', 'losses', 'draws', 'byes', 'hbyes', 'winrate', 'status'], 
                           show='headings', height=15)
        ...
        for i, player in enumerate(sorted_players, 1):
            status = "🏆 Winner" if i == 1 else ("❌ Eliminated" if player.eliminated else "Active")
            tree.insert('', tk.END, values=(
                i, player.name, player.rating, player.wins, player.losses, 
                player.draws, player.byes, player.half_byes, f"{player.win_rate:.1f}%", status
            ))
    ...
    def show_dual_results(self, pairs: List[Tuple[Player, Player]], leftover: Player = None):
        ...
        for i, (p1, p2) in enumerate(pairs, 1):
            pair_frame = ttk.LabelFrame(scrollable_frame, text=f"Match {i}", padding="10")
            pair_frame.pack(fill=tk.X, padx=5, pady=5)
            
            ttk.Label(pair_frame, text=f"{p1.name} ({rating_name}: {p1.rating})", 
                     font=('Arial', 11)).pack(anchor=tk.W)
            ttk.Label(pair_frame, text="vs", font=('Arial', 10, 'italic')).pack(anchor=tk.W, padx=20)
            ttk.Label(pair_frame, text=f"{p2.name} ({rating_name}: {p2.rating})", 
                     font=('Arial', 11)).pack(anchor=tk.W)
        
        if leftover:
            leftover_frame = ttk.LabelFrame(scrollable_frame, text="No Match", padding="10")
            leftover_frame.pack(fill=tk.X, padx=5, pady=5)
            ttk.Label(leftover_frame, text=f"{leftover.name} ({rating_name}: {leftover.rating})", 
                     font=('Arial', 11)).pack(anchor=tk.W)
        ...
        ttk.Button(btn_frame, text="← Back to Input", command=self.show_player_input).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Regenerate", command=self.generate_results).pack(side=tk.LEFT, padx=5)
    ...
    def show_battle_royale_results(self, sorted_players: List[Player]):
        ...
        # Title
        title = ttk.Label(frame, text="Battle Royale Rankings", font=('Arial', 18, 'bold'))
        title.pack(pady=10)
        ...
        tree = ttk.Treeview(results_frame, columns=['rank', 'name', 'rating', 'winrate'], 
                           show='headings', height=15)
        ...
        for i, player in enumerate(sorted_players, 1):
            tree.insert('', tk.END, values=(i, player.name, player.rating, f"{player.win_rate:.1f}%"))
    ...
    def show_team_configuration(self):
        ...
        if len(self.players) < 2:
            messagebox.showwarning("Not Enough Players", "Need at least 2 players for team mode")
            return
        ...
        spinbox = ttk.Spinbox(frame, from_=2, to=min(10, len(self.players)), 
                             textvariable=num_teams_var, width=10)
    ...
    def show_team_game(self):
        ...
        # Title
        title = ttk.Label(frame, text=f"Teams Mode - Round {self.current_round}", 
                         font=('Arial', 16, 'bold'))
        title.pack(pady=10)
        ...
        for i, team in enumerate(self.teams, 1):
            ...
            team_frame = ttk.LabelFrame(scrollable_frame, 
            ...
            for player in team:
                ...
                ttk.Label(player_frame, text=info_text, font=('Arial', 9), width=40).pack(side=tk.LEFT)
                
                # Action buttons
                btn_frame = ttk.Frame(player_frame)
                btn_frame.pack(side=tk.RIGHT)
                
                ttk.Button(btn_frame, text="+W", width=3,
                          command=lambda p=player: self.add_team_result(p, 'win')).pack(side=tk.LEFT, padx=1)
                ttk.Button(btn_frame, text="+L", width=3,
                          command=lambda p=player: self.add_team_result(p, 'loss')).pack(side=tk.LEFT, padx=1)
                ttk.Button(btn_frame, text="+D", width=3,
                          command=lambda p=player: self.add_team_result(p, 'draw')).pack(side=tk.LEFT, padx=1)
        ...
        ttk.Button(btn_frame, text="← Back to Setup", 
                  command=self.back_to_setup).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Finish Round", 
                  command=self.finish_team_round).pack(side=tk.LEFT, padx=5)
    ...
    def show_team_standings(self):
        ...
        # Title
        title = ttk.Label(frame, text=f"Teams Mode - Standings After Round {self.current_round}", 
                         font=('Arial', 16, 'bold'))
        title.pack(pady=10)
        ...
        for rank, (team_num, team, total_wr, avg_rating, mvp) in enumerate(team_stats, 1):
            team_frame = ttk.LabelFrame(scrollable_frame, 
            ...
            if mvp:
                ...
                ttk.Label(mvp_frame, text="🏅 MVP:", font=('Arial', 10, 'bold')).pack(side=tk.LEFT)
                ttk.Label(mvp_frame, 
            ...
            for player in sorted(team, key=lambda p: p.win_rate, reverse=True):
                player_text = f"  • {player.name} - {rating_name}: {player.rating}, " \
                            f"WR: {player.win_rate:.1f}% ({player.wins}W-{player.losses}L-{player.draws}D)"
                ttk.Label(team_frame, text=player_text, font=('Arial', 9)).pack(anchor=tk.W, pady=1)
        ...
        if self.max_rounds and self.current_round >= self.max_rounds:
            # Max rounds reached - auto finish
            ttk.Button(btn_frame, text="View Final Standings",
                      command=self.show_team_final_standings).pack(side=tk.LEFT, padx=5)
        else:
            # Can continue
            ttk.Button(btn_frame, text="Next Round", 
                      command=self.next_team_round).pack(side=tk.LEFT, padx=5)
            ttk.Button(btn_frame, text="Finish Tournament",
                      command=self.show_team_final_standings).pack(side=tk.LEFT, padx=5)
    ...
    def show_team_final_standings(self):
        ...
        # Title
        title = ttk.Label(frame, text=f"Teams Mode - Final Standings", 
                         font=('Arial', 18, 'bold'))
        title.pack(pady=10)
        ...
        if team_stats:
            ...
            ttk.Label(frame, text=f"🏆 Winning Team: Team {winning_team[0]} 🏆",
                     font=('Arial', 16, 'bold')).pack(pady=10)
    ...
    def calculate_elo_change(self, player_rating: int, opponent_rating: int, result: float, mode: str = "otb") -> int:
        """
    ...
    def apply_automatic_elo_changes(self):
        ...
        for pairing, result_var in self.tournament_results:
            ...
            p1_change = self.calculate_elo_change(p1.rating, p2.rating, p1_result, elo_mode)
            p2_change = self.calculate_elo_change(p2.rating, p1.rating, p2_result, elo_mode)
    ...
    def show_manual_rating_update(self, callback):
        ...
        rating_name = "ELO" if self.game_type == 'chess' else "Trophies"
        title = ttk.Label(frame, text=f"Update {rating_name} Ratings", font=('Arial', 16, 'bold'))
        title.pack(pady=10)
        
        ttk.Label(frame, text=f"Manually adjust {rating_name} ratings based on round performance:", 
                 font=('Arial', 10)).pack(pady=5)
        ...
        active_players = [p for p in self.players if not p.eliminated and not p.withdrawn]
        for player in sorted(active_players, key=lambda p: p.points, reverse=True):
            ...
            ttk.Label(player_frame, text=info_text, width=35, anchor=tk.W).pack(side=tk.LEFT, padx=5)
            
            # Current rating
            ttk.Label(player_frame, text=f"Current {rating_name}:").pack(side=tk.LEFT, padx=5)
            ttk.Label(player_frame, text=str(player.rating), width=8).pack(side=tk.LEFT)
            
            # New rating input
            ttk.Label(player_frame, text=f"→ New {rating_name}:").pack(side=tk.LEFT, padx=10)
        ...
        ttk.Button(btn_frame, text="Skip (No Changes)", 
                  command=callback).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Apply Changes", 
                  command=lambda: self.apply_manual_ratings(callback)).pack(side=tk.LEFT, padx=5)
    ...
    def show_swiss_round(self):
        ...
        title = ttk.Label(frame, text=f"Swiss System - Round {self.current_round}", 
                         font=('Arial', 16, 'bold'))
        title.pack(pady=10)
        ...
        if self.current_round == 1:
            ...
            for player in playing_players:
                ...
                for candidate in playing_players:
                    if (candidate.name != player.name and 
                        candidate.name not in paired and
                        candidate.name not in player.opponents):
                        opponent = candidate
                        break
    ...
    def show_round_robin_round(self):
        ...
        title = ttk.Label(frame, text=f"Round-Robin - Round {self.current_round}", 
                         font=('Arial', 16, 'bold'))
        title.pack(pady=10)
    ...
    def show_knockout_round(self):
        ...
        round_names = {1: "Round 1", 2: "Round 2", 3: "Quarterfinals", 
                      4: "Semifinals", 5: "Finals"}
        round_name = round_names.get(self.current_round, f"Round {self.current_round}")
        
        title = ttk.Label(frame, text=f"Knockout - {round_name}", 
                         font=('Arial', 16, 'bold'))
        title.pack(pady=10)
    ...
    def show_scheveningen_round(self):
        ...
        title = ttk.Label(frame, text=f"Scheveningen - Round {self.schev_round}/{self.schev_total_rounds}", 
                         font=('Arial', 16, 'bold'))
        title.pack(pady=10)
        ...
        for i in range(len(playing_team_a)):
            if i < len(playing_team_b):
                ...
                if opponent_idx < len(playing_team_b):
                    pairings.append([playing_team_a[i], playing_team_b[opponent_idx], None])
    ...
    def show_scheveningen_standings(self):
        ...
        title = ttk.Label(frame, text=f"Scheveningen - Standings After Round {self.schev_round}",
                         font=('Arial', 16, 'bold'))
        title.pack(pady=10)
        ...
        tree = ttk.Treeview(results_frame,
                           columns=['rank', 'team', 'name', 'rating', 'points', 'wins', 'losses', 'draws', 'byes', 'hbyes', 'tiebreak'],
        ...
        for i, (player, tb_score) in enumerate(sorted_players, 1):
            ...
            tree.insert('', tk.END, values=(
                i, team, player.name, player.rating, player.points,
                player.wins, player.losses, player.draws, player.byes, player.half_byes,
        ...
        if self.half_bye_enabled:
            halfbye_frame = ttk.LabelFrame(frame, text="Half-Bye Requests for Next Round", padding="10")
            halfbye_frame.pack(fill=tk.X, pady=5, padx=10)
            
            ttk.Label(halfbye_frame, 
            ...
            active_players = [p for p in all_players if not p.eliminated and not p.withdrawn]
        ...
        if self.withdrawal_enabled:
            withdrawal_frame = ttk.LabelFrame(frame, text="Player Withdrawals", padding="10")
            withdrawal_frame.pack(fill=tk.X, pady=5, padx=10)
            
            ttk.Label(withdrawal_frame, 
        ...
        if self.schev_round >= self.schev_total_rounds:
            ttk.Button(btn_frame, text="View Final Standings",
                      command=self.show_scheveningen_final).pack(side=tk.LEFT, padx=5)
        else:
            ttk.Button(btn_frame, text="Next Round",
                      command=self.next_scheveningen_round).pack(side=tk.LEFT, padx=5)
            ttk.Button(btn_frame, text="Finish Tournament",
                      command=self.show_scheveningen_final).pack(side=tk.LEFT, padx=5)
    ...
    def show_scheveningen_final(self):
        ...
        title = ttk.Label(frame, text="Scheveningen - Final Results", 
                         font=('Arial', 18, 'bold'))
        title.pack(pady=10)
        ...
        # Team A
        team_a_frame = ttk.LabelFrame(teams_frame, text=f"Team A ({team_a_score} points)", padding="10")
        ...
        for p in sorted(self.schev_team_a, key=lambda x: x.points, reverse=True):
            ttk.Label(team_a_frame, 
                     text=f"{p.name}: {p.points} pts ({p.wins}W-{p.losses}L-{p.draws}D)",
                     font=('Arial', 10)).pack(anchor=tk.W, pady=2)
        
        # Team B
        team_b_frame = ttk.LabelFrame(teams_frame, text=f"Team B ({team_b_score} points)", padding="10")
        ...
        for p in sorted(self.schev_team_b, key=lambda x: x.points, reverse=True):
            ttk.Label(team_b_frame, 
                     text=f"{p.name}: {p.points} pts ({p.wins}W-{p.losses}L-{p.draws}D)",
                     font=('Arial', 10)).pack(anchor=tk.W, pady=2)
        
        ttk.Button(frame, text="← Back to Setup", command=self.back_to_setup).pack(pady=20)
    ...
    def display_tournament_pairings(self, parent_frame, pairings, system):
        ...
        for i, pairing in enumerate(pairings):
            ...
            pair_frame = ttk.LabelFrame(scrollable_frame, text=f"Board {i+1}", padding="10")
            pair_frame.pack(fill=tk.X, padx=5, pady=5)
        ...
        ttk.Button(btn_frame, text="← Back to Setup",
                  command=self.back_to_setup).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Finish Round",
                  command=lambda: self.finish_tournament_round(system)).pack(side=tk.LEFT, padx=5)
    ...
    def show_tournament_standings(self, system):
        ...
        title = ttk.Label(frame, text=f"{system_name} - Standings After Round {self.current_round}",
                         font=('Arial', 16, 'bold'))
        title.pack(pady=10)
        ...
        tree = ttk.Treeview(results_frame,
                           columns=['rank', 'name', 'rating', 'points', 'wins', 'losses', 'draws', 'byes', 'hbyes', 'tiebreak'],
                           show='headings', height=12)
        ...
        for i, (player, tb_score) in enumerate(sorted_players, 1):
            tree.insert('', tk.END, values=(
                i, player.name, player.rating, player.points,
                player.wins, player.losses, player.draws, player.byes, player.half_byes,
                f"{tb_score:.1f}" if tb_score else "-"
            ))
        ...
        if self.half_bye_enabled:
            ...
            for i, player in enumerate([p for p in self.players if not p.eliminated and not p.withdrawn]):
                if i % players_per_row == 0:
                    row_frame = ttk.Frame(halfbye_frame)
                    row_frame.pack(fill=tk.X, pady=2)
        ...
        if should_auto_finish:
            ...
            ttk.Button(btn_frame, text="View Final Standings",
                      command=self.show_tournament_final_standings).pack(side=tk.LEFT, padx=5)
        else:
            # Can continue - show next round button
            ttk.Button(btn_frame, text="Next Round",
                      command=lambda: self.next_tournament_round_with_settings(system)).pack(side=tk.LEFT, padx=5)
    ...
    def apply_tiebreak(self, players):
        ...
        result.sort(key=lambda x: (x[0].points, x[1] if x[1] is not None else 0), reverse=True)
        ...
        withdrawn_sorted = sorted(withdrawn_players, 
    ...
    def show_tournament_final_standings(self):
        ...
        title = ttk.Label(frame, text=f"{system_name} - Final Standings",
                         font=('Arial', 18, 'bold'))
        title.pack(pady=10)
        ...
        tree = ttk.Treeview(results_frame,
                           columns=['rank', 'name', 'rating', 'points', 'record', 'status', 'tiebreak'],
                           show='headings', height=15)
        ...
        for i, (player, tb_score) in enumerate(sorted_players, 1):
            ...
            tree.insert('', tk.END, values=(
                i, player.name, player.rating, player.points, record, status,
                f"{tb_score:.1f}" if tb_score else "-"
            ))
        ...
        ttk.Button(frame, text="← Back to Setup", command=self.back_to_setup).pack(pady=10)
    ...
    def show_tournament_winner(self, winner):
        ...
        ttk.Label(frame, text="🏆 TOURNAMENT WINNER! 🏆",
        ...
        ttk.Label(winner_frame, text=winner.name, font=('Arial', 20, 'bold')).pack(pady=10)
        ttk.Label(winner_frame, text=f"ELO: {winner.rating}",
                 font=('Arial', 14)).pack(pady=5)
        ttk.Label(winner_frame, text=f"Points: {winner.points}",
                 font=('Arial', 14)).pack(pady=5)
    ...
    def back_to_setup(self):
        ...
        if self.in_game:
            if messagebox.askyesno("Confirm", "Return to setup? Current game progress will be kept."):
                self.in_game = False
                self.show_player_input()
        else:
            self.show_player_input()
